...
function Type() {
  ...
  <div className={style.mainPage} id="mainPage">
      {/* <!-- 첫번째 질문, 카테고리1 --> */}
...
아이돌 타입은 상대를 기쁘게 하는 것에 보람을 느끼기 때문에,우울할 때일수록 사람들과의 교류가 필요합니다.
    ...
    <div className={style.resultTeletTox} style="textTlign: center">
      <button className={`${style.resultBox} ${style.tryAgainBtn}`} id={style.tryAgainBtn} onClick="window.location.reload()">
        테스트 다시 하기
      </button>
  
      <button className={`${style.resultBox} ${style.shoppingBtn}`} id={style.shoppingBtn}  onclick="location.href='/productTist/product.html';">
        다른 제품 보러가기
        </button>
