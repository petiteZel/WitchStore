...
// 1. 카테고리등록 api (아래는 /category/register이지만, 실제로는 /api/category/register로 요청해야 함.)
categoryRouter.post('/category/register', adminOnly, async (req, res, next) => {
  try {
...
// 카테고리 정보 삭제
categoryRouter.delete('/category/delete/:categoryId', async function (req, res, next) {
  try {
    const categoryId = req.params.categoryId;
